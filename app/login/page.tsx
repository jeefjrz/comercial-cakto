...
export default function LoginPage() {
  ...
  const inp: React.CSSProperties = {
    ...
    color: '#0f172a', fontSize: '15px', outline: 'none', fontFamily: 'inherit',
  };
    ...
    <div style={{ minHeight: '100vh', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ background: '#ffffff', borderRadius: '20px', padding: '40px', width: '100%', maxWidth: '420px', boxShadow: '0 10px 40px rgba(0,0,0,0.12)', border: '1px solid #e2e8f0' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'linear-gradient(135deg,#2563eb,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px', fontSize: '24px' }}>⚡</div>
          <div style={{ fontSize: '22px', fontWeight: 800, color: '#0f172a' }}>Comercial Cakto</div>
          <div style={{ fontSize: '13px', color: '#64748b', marginTop: '4px' }}>Sistema Comercial Interno</div>
        </div>
        ...
        <form onSubmit={tab === 'login' ? handleLogin : handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {tab === 'register' && (
            <div>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#374151', marginBottom: '6px' }}>Confirmar Senha</label>
